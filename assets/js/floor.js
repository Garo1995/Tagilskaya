...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let floorCountSwiper;
    let floorSwiper;

    // Принимаем swiperInstance (this) внутрь
    function syncFloorNumberAndSlider(swiperInstance) {
        const realIndex = swiperInstance.realIndex;

        const realSlides = document.querySelectorAll(".count-slid .swiper-slide:not(.swiper-slide-duplicate)");
        const realSlide = realSlides[realIndex];
        if (!realSlide) return;

        const numberEl = realSlide.querySelector(".count-slid-floor");
        if (!numberEl) return;

        const floorNumber = numberEl.textContent.trim();

        activeFloorSpans.forEach(span => {
            span.textContent = floorNumber;
        });

        // Синхронизируем второй слайдер
        if (floorSwiper) {
            floorSwiper.slideToLoop(realIndex);
        }
    }

    // Основной слайдер с изображением этажей
    floorSwiper = new Swiper(".floor-plan-slider", {
        slidesPerView: 1,
        loop: true,
    ...
    // Слайдер с цифрами этажей
    floorCountSwiper = new Swiper(".count-slid", {
        slidesPerView: 3,
        direction: "vertical",
        loop: true,
        spaceBetween: 10,
        ...
        on: {
            init: syncFloorNumberAndSlider,
            slideChange: syncFloorNumberAndSlider,
        },
    });
});
...
$('.see-plan').on('click', function () {
    $('body').addClass('body-hidden')
    $('.found-floor').addClass('found-floor-active')
})
$('.parameters-btn').on('click', function () {
    $('body').addClass('body-hidden')
    $('.found-floor').addClass('found-floor-active')
})
...
$('.open-all-floor').on('click', function () {
...
$('.buy-floor-one').on('click', function () {
    $('.floor-premises-square').addClass('premises-square-active')
})
...
$(document).ready(function () {
    $('.get-title').on('click', function () {
        const titleText = $('.get-title-mobal').text();
        $('.send-title').text(titleText);
        $('.clicking-submit-form').addClass('title-actives');
    });
...
$(document).ready(function () {
    $('g.buy-floor-one').on('mouseenter', function () {
        ...
        $('.numbers-floor').text(floor);
        $('.count-square').text(square);
        $('.price-floors').text(price);
    });
});
